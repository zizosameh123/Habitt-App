...
class HabitTracker:
    ...
    def save_to_json(self, filename):
        ...
        for cat_name, category in self.categories.items():
            data[cat_name] = {
                "habits": {
                    habit.name: {"count": habit.count, "schedules": habit.schedules}
...
class App(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(self.habit_frame, text="Habits", font=("Arial", 24, "bold")).grid(
        ...
        self.categories_frame.grid(row=2, column=0, sticky="nsew", padx=10, pady=10)

        # Schedule Section
        self.schedule_frame = ctk.CTkFrame(self)
        self.schedule_frame.grid(row=0, column=1, sticky="nsew", padx=10, pady=10)
        self.schedule_frame.grid_columnconfigure(0, weight=1)
        self.schedule_frame.grid_rowconfigure(3, weight=1)

        ctk.CTkLabel(
            self.schedule_frame, text="Schedules", font=("Arial", 24, "bold")
        ).grid(row=0, column=0, pady=10)

        self.create_schedule_input_frame()

        self.schedule_list = ctk.CTkTextbox(self.schedule_frame)
        self.schedule_list.grid(row=3, column=0, sticky="nsew", padx=10, pady=10)
    ...
    def create_habit_input_frame(self):
        ...
        self.category_input.bind("<Return>", lambda event: self.add_category())

        ctk.CTkButton(input_frame, text="Add Category", command=self.add_category).grid(
            row=0, column=2, padx=5, pady=5, sticky="ew"
        )
        ctk.CTkButton(
            input_frame, text="Remove Category", command=self.remove_category
        ).grid(row=0, column=3, padx=5, pady=5, sticky="ew")

        self.habit_input = ctk.CTkEntry(input_frame, placeholder_text="Enter habit")
        self.habit_input.grid(row=1, column=0, padx=5, pady=5, sticky="ew")
        self.habit_input.bind("<Return>", lambda event: self.add_habit())
    ...
    def create_schedule_input_frame(self):
        ...
        self.schedule_habit_input.grid(row=0, column=0, padx=5, pady=5, sticky="ew")
        self.schedule_habit_input.bind("<Return>", lambda event: self.add_schedule())

        self.schedule_time_input = ctk.CTkEntry(
            input_frame, placeholder_text="Enter time (HH:MM)"
        )
        self.schedule_time_input.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
        self.schedule_time_input.bind("<Return>", lambda event: self.add_schedule())
    ...
    def remove_category(self):
        ...
        if messagebox.askyesno(
            "Confirm Deletion",
            f"Are you sure you want to delete the category '{category_name}' and all its habits?",
        ):
    ...
    def update_category_list(self):
        ...
        for category_name, category in self.habit_tracker.categories.items():
            ...
            ctk.CTkLabel(
                category_frame, text=category_name, font=("Arial", 18, "bold")
            ).pack()
            ...
            for habit_name, habit in category.habits.items():
                ...
                ctk.CTkLabel(
                    habit_frame, text=habit_info, wraplength=400, justify="left"
                ).pack(side="left", padx=5)
                ctk.CTkButton(
                    habit_frame,
                    text="Finished",
                    command=lambda h=habit: self.increment_habit(h),
        ...
        for category in self.habit_tracker.categories.values():
            for habit in category.habits.values():
                for time in habit.schedules:
                    self.schedule_list.insert("end", f"{habit.name} - Time: {time}\n")
    ...
    def load_data(self):
        if not self.habit_tracker.load_from_json("habit_data.json"):
            messagebox.showinfo("Welcome", "Welcome to Phantom's Habit Tracker!")
    ...
    def check_notifications(self):
        while True:
            ...
            for category in self.habit_tracker.categories.values():
                for habit in category.habits.values():
                    if now in habit.schedules:
                        messagebox.showinfo("Reminder", f"It's time to {habit.name}!")
            time.sleep(60)
